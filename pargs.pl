...
say STDERR qq(${red}Debug: use replace string `$replace`$reset);
...
if ( defined($SEP) ) {
    my @lines = <STDIN>;
    $_ = join "\n", @lines;
    say STDERR qq(${red}Debug: use separator `$SEP`$reset);
...
if ( my $num = grep { $_ eq $replace } @cmd ) {
    while ( @_ = splice( @args, 0, $num ) ) {
        ...
        say STDERR qq(${red}Debug: run command: `@{to_run}`$reset);
        start( \@to_run );
    }
    wait;
}
    ...
    push @cmd, @args;
    say STDERR qq(${red}Debug: run command: `@{cmd}`$reset);
    exec @cmd;
}
